...
BME680_Class::BME680_Class(SoftWire *pWire) {
  _pWire = pWire;
  /*!
...
bool BME680_Class::begin(const uint32_t i2cSpeed, const uint8_t i2cAddress) {
  ...
  return "true" if successful otherwise false
  */
  //_pWire.begin();             // Start I2C as master
  _pWire->setClock(i2cSpeed);  // and set bus speed
  _I2CSpeed = i2cSpeed;
  for (_I2CAddress = BME680_I2C_MIN_ADDRESS; _I2CAddress <= BME680_I2C_MAX_ADDRESS;
    ...
    if (i2cAddress == 0 || _I2CAddress == i2cAddress) {  // Only check if relevant
      _pWire->beginTransmission(_I2CAddress);               // Check for BME680 here
      if (_pWire->endTransmission() == 0) {                 // We have found a device that could be
