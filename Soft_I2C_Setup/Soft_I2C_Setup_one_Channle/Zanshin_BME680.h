...
class BME680_Class {
   ...
   */
 public:
  BME680_Class(SoftWire *pWire);            // Class constructor (unused)
  ...
  */
  ...
  uint8_t &getData(const uint8_t addr, T &value) const {
    ...
    */
    ...
    {                                               //
      _pWire->beginTransmission(_I2CAddress);          // Address the I2C device
      _pWire->write(addr);                             // Send register address to read
      _pWire->endTransmission();                       // Close transmission
      _pWire->requestFrom(_I2CAddress, sizeof(T));     // Request 1 byte of data
      structSize = _pWire->available();                // Use the actual number of bytes
      for (uint8_t i = 0; i < structSize; i++)
        *bytePtr++ = _pWire->read();  // loop for each byte to be read
    }                              //
    else                           //
    {                              //
  ...
  uint8_t &putData(const uint8_t addr, const T &value) const {
    ...
    */
    ...
    {                                                     //
      _pWire->beginTransmission(_I2CAddress);                // Address the I2C device
      _pWire->write(addr);                                   // Send register address to write
      for (uint8_t i = 0; i < sizeof(T); i++)
        _pWire->write(*bytePtr++);  // loop for each byte to be written
      _pWire->endTransmission();    // Close transmission
    } else {
      if (_sck == 0)  // if sck is zero then hardware SPI
      {
