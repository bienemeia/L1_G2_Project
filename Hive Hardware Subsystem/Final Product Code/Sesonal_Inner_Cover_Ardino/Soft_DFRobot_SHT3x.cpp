...
Soft_DFRobot_SHT3x::Soft_DFRobot_SHT3x(SoftWire *pWire, uint8_t address,uint8_t RST)
{
  _pWire = pWire;
  _address = address;
  _RST = RST;
...
uint32_t Soft_DFRobot_SHT3x::readSerialNumber()
{
  uint32_t result = 0 ;
...
bool Soft_DFRobot_SHT3x::softReset()
{
...
bool Soft_DFRobot_SHT3x::pinReset()
{
  sStatusRegister_t registerRaw;
  clearStatusRegister();
...
bool Soft_DFRobot_SHT3x::stopPeriodicMode()
{  
  sStatusRegister_t registerRaw;
  measurementMode = eOneShot;
...
bool Soft_DFRobot_SHT3x::heaterEnable()
{
  sStatusRegister_t registerRaw;
...
bool Soft_DFRobot_SHT3x::heaterDisable()
{
  sStatusRegister_t registerRaw;
...
void Soft_DFRobot_SHT3x::clearStatusRegister(){
  writeCommand(SHT3X_CMD_CLEAR_STATUS_REG,2);
  delay(10);
}

bool Soft_DFRobot_SHT3x::readAlertState()
{
  sStatusRegister_t registerRaw;
  delay(1);
  registerRaw = readStatusRegister();
...
Soft_DFRobot_SHT3x::sRHAndTemp_t Soft_DFRobot_SHT3x::readTemperatureAndHumidity(eRepeatability_t repeatability)
{
  uint8_t rawData[6];
...
float Soft_DFRobot_SHT3x::getTemperatureC(){
...
float Soft_DFRobot_SHT3x::getTemperatureF()
{
  if(measurementMode == eOneShot){
    readTemperatureAndHumidity(eRepeatability_High);
  } else {
...
float Soft_DFRobot_SHT3x::getHumidityRH()
{
  if(measurementMode == eOneShot){
    readTemperatureAndHumidity(eRepeatability_High);
  } else {
...
bool Soft_DFRobot_SHT3x::startPeriodicMode(eMeasureFrequency_t measureFreq,eRepeatability_t repeatability)
{
...
Soft_DFRobot_SHT3x::sStatusRegister_t Soft_DFRobot_SHT3x::readStatusRegister(){
  uint8_t register1[3];
  uint16_t data;
  sStatusRegister_t registerRaw;
  uint8_t retry = 10;
...
uint8_t Soft_DFRobot_SHT3x::environmentState()
{ 
  sStatusRegister_t registerRaw;
  float rhHighSet ;
  float rhLowSet ;
...
Soft_DFRobot_SHT3x::sRHAndTemp_t Soft_DFRobot_SHT3x::readTemperatureAndHumidity()
{
  uint8_t rawData[6];
  uint8_t rawTemperature[3];
  uint8_t rawHumidity[3];
...
uint8_t  Soft_DFRobot_SHT3x::setTemperatureLimitC(float highset,float highclear, float lowset,float lowclear)
{
...
uint8_t Soft_DFRobot_SHT3x::setTemperatureLimitF(float highset,float highclear,float lowset,float lowclear)
{
...
uint8_t Soft_DFRobot_SHT3x::setHumidityLimitRH(float highset,float highclear, float lowset,float lowclear)
{
  uint16_t _highset ,_highclear,_lowclear,_lowset,limit[1];
  if(highset > highclear && highclear > lowclear && lowclear> lowset){
...
bool Soft_DFRobot_SHT3x::measureTemperatureLimitC(){
...
bool Soft_DFRobot_SHT3x::measureTemperatureLimitF()
{
  uint16_t limit[1] ;
  float data;
  if(readLimitData(SHT3X_CMD_READ_HIGH_ALERT_LIMIT_SET,limit) != 0) {
...
float Soft_DFRobot_SHT3x::getTemperatureHighSetF(){
  return limitData.highSet;
}

float Soft_DFRobot_SHT3x::getTemperatureHighClearF(){
  return limitData.highClear;
}

float Soft_DFRobot_SHT3x::getTemperatureLowClearF(){
  return limitData.lowClear;
}
float Soft_DFRobot_SHT3x::getTemperatureLowSetF(){
  return limitData.lowSet;
}
uint8_t Soft_DFRobot_SHT3x::readLimitData(uint16_t cmd,uint16_t *pBuf)
{ 
  uint8_t rawData[3];
  uint8_t crc;
...
float Soft_DFRobot_SHT3x::getTemperatureHighSetC(){
  return limitData.highSet;
}
float Soft_DFRobot_SHT3x::getTemperatureHighClearC(){
  return limitData.highClear;
}
float Soft_DFRobot_SHT3x::getTemperatureLowClearC(){
  return limitData.lowClear;
}
float Soft_DFRobot_SHT3x::getTemperatureLowSetC(){
  return limitData.lowSet;
}
float Soft_DFRobot_SHT3x::getHumidityHighSetRH(){
  return limitData.highSet;
}
float Soft_DFRobot_SHT3x::getHumidityHighClearRH(){
  return limitData.highClear;
}
float Soft_DFRobot_SHT3x::getHumidityLowClearRH(){
  return limitData.lowClear;
}
float Soft_DFRobot_SHT3x::getHumidityLowSetRH(){
  return limitData.lowSet;
}

bool Soft_DFRobot_SHT3x::measureHumidityLimitRH()
{
...
float Soft_DFRobot_SHT3x::convertTemperature(uint8_t rawTemperature[])
{
  uint16_t rawValue ;
...
float Soft_DFRobot_SHT3x::convertHumidity(uint8_t rawHumidity[])
{
  uint16_t rawValue ;
...
uint16_t Soft_DFRobot_SHT3x::convertRawTemperature(float value)
{
  return (value + 45.0f) / 175.0f * 65535.0f;
}

uint16_t Soft_DFRobot_SHT3x::convertRawHumidity(float value)
{
  return value / 100.0f * 65535.0f;
}

float Soft_DFRobot_SHT3x::convertTempLimitData(uint16_t limit[])
{
  limit[1] = limit[1] << 7;
...
float Soft_DFRobot_SHT3x::convertHumidityLimitData(uint16_t limit[])
{
...
uint8_t Soft_DFRobot_SHT3x::checkCrc(uint8_t data[])
{
    uint8_t bit;
    uint8_t crc = 0xFF;
...
void Soft_DFRobot_SHT3x::writeLimitData(uint16_t cmd,uint16_t limitData){
  uint8_t _pBuf[5];
  _pBuf[0] = cmd >>8;
  _pBuf[1] = cmd & 0xff;
  _pBuf[2] = limitData >> 8;
...
void Soft_DFRobot_SHT3x::writeCommand(uint16_t cmd,size_t size)
{
  uint8_t _pBuf[2];
  _pBuf[0] = cmd >> 8;
  _pBuf[1] = cmd & 0xFF;
...
void Soft_DFRobot_SHT3x::write(const void* pBuf,size_t size)
{
  if (pBuf == NULL) {
...
uint8_t Soft_DFRobot_SHT3x::readData(void *pBuf, size_t size) {
  if (pBuf == NULL) {
    DBG("pBuf ERROR!! : null pointer");
  }
  uint8_t * _pBuf = (uint8_t *)pBuf;
